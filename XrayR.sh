...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/dockerps13/starbridge-xrayr-script/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontent.com/dockerps13/starbridge-xrayr-script/master/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
...
# 0: connected, 1: not running, 2: not installed, 3: not connected
check_panel_status() {
...
show_status() {
    ...
    case $? in
        ...
        2)
            ...
            ;;
        3)
            echo -e "XrayR状态: ${yellow}未连接${plain}"
            show_enable_status
    esac
}
...
--- https://github.com/dockerps13/starbridge-xrayr ---
  ${green}0.${plain} 修改配置
————————————————
...
————————————————
 ${green}11.${plain} 应用安全网络优化 (原生 BBR/fq)
